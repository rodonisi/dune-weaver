...
async function selectFile(file, listItem) {
  ...
  document
    .querySelectorAll("#theta_rho_files li")
    .forEach((li) => li.classList.remove("selected"));
  listItem.classList.add("selected");

  // Update the Remove button visibility
  const removeButton = document.querySelector(
    "#pattern-preview-container .remove-button"
  );
...
function searchPatternFiles() {
  const searchInput = document
    .getElementById("search_pattern")
    .value.toLowerCase();
  const filteredFiles = allFiles.filter((file) =>
    file.toLowerCase().includes(searchInput)
  );
...
async function runThetaRho() {
  ...
  logMessage(
    `Running file: ${selectedFile} with pre-execution action: ${preExecutionAction}...`
  );
  try {
    ...
    if (response.ok) {
      ...
      const currentlyPlayingFile = document.getElementById(
        "currently-playing-file"
      );
      if (currentlyPlayingFile) {
        currentlyPlayingFile.textContent = selectedFile.replace(
          "./patterns/",
          ""
        );
      }
      ...
      if (response.status === 409) {
        logMessage(
          "Cannot start pattern: Another pattern is already running",
          LOG_TYPE.WARNING
        );
      } else {
        logMessage(
          `Failed to run file: ${result.detail || "Unknown error"}`,
          LOG_TYPE.ERROR
        );
      }
    }
...
async function removeCustomPattern(fileName) {
  const userConfirmed = confirm(
    `Are you sure you want to delete the pattern "${fileName}"?`
  );
  ...
  try {
    ...
    if (result.success) {
      logMessage(
        `File deleted successfully: ${selectedFile}`,
        LOG_TYPE.SUCCESS
      );
      ...
      logMessage(
        `Failed to delete pattern "${fileName}": ${result.error}`,
        LOG_TYPE.ERROR
      );
    }
  } catch (error) {
  ...
  try {
    ...
    if (data.first_coordinate && data.last_coordinate) {
      coordDisplay.innerHTML = `
                <div>First Coordinate: θ=${data.first_coordinate[0].toFixed(
                  2
                )}, ρ=${data.first_coordinate[1].toFixed(2)}</div>
                <div>Last Coordinate: θ=${data.last_coordinate[0].toFixed(
                  2
                )}, ρ=${data.last_coordinate[1].toFixed(2)}</div>
            `;
    } else {
...
async function updateCurrentlyPlayingPattern(fileName) {
  try {
    ...
    logMessage(
      `Error updating currently playing pattern: ${error.message}`,
      LOG_TYPE.ERROR
    );
  }
}
...
function renderPattern(coordinates, canvasId) {
  ...
  if (!ctx) {
    logMessage(
      `Could not get 2D context for canvas: ${canvasId}`,
      LOG_TYPE.ERROR
    );
    return;
  }
...
async function sendCoordinate() {
  ...
  if (result.success) {
    logMessage(
      `Coordinate executed successfully: θ=${theta}, ρ=${rho}`,
      LOG_TYPE.SUCCESS
    );
  } else {
...
function executeClearAction(actionFunction) {
  ...
  if (actionFunction && typeof window[actionFunction] === "function") {
    ...
    logMessage(
      "No clear action selected or function not found.",
      LOG_TYPE.ERROR
    );
  }
}
...
async function runFile(fileName) {
  const response = await fetch(`/run_theta_rho_file/${fileName}`, {
    method: "POST",
  });
  const result = await response.json();
  if (result.success) {
...
async function checkSerialStatus() {
  ...
  const statusHeaderElement = document.getElementById(
    "connection_status_header"
  );
  const serialPortsContainer = document.getElementById(
...
async function connectSerial() {
  ...
  if (result.success) {
    ...
    logMessage(
      `Error connecting to serial port: ${result.error}`,
      LOG_TYPE.ERROR
    );
  }
}
...
async function restartSerial() {
  ...
  if (result.success) {
    document.getElementById(
      "serial_status"
    ).textContent = `Restarted connection to ${port}`;
...
async function checkForUpdates() {
  try {
    ...
    currentVersionElem.textContent = `Current Version: ${
      data.latest_local_tag || "Unknown"
    }`;
...
// Add a helper function to display current schedule status
function updateScheduleStatus() {
  ...
  if (startTime && endTime) {
    ...
    const statusText = isActive
      ? `<i class="fa-solid fa-clock"></i> Scheduled ${startTime}-${endTime} (Currently ACTIVE)`
      : `<i class="fa-solid fa-clock"></i> Scheduled ${startTime}-${endTime} (Currently INACTIVE)`;

    scheduleInfo.innerHTML = `<small style="color: ${
      isActive ? "var(--color-success)" : "var(--color-warning)"
    }">${statusText}</small>`;
  } else {
    scheduleInfo.innerHTML =
      '<small><i class="fa-solid fa-info-circle"></i> When scheduling is enabled, the playlist will only run during the specified time range. Use 24-hour format (e.g., 22:00 to 06:00 for overnight).</small>';
  }
}

// Add event listeners to update schedule status when times change
...
async function runPlaylist() {
  ...
  const pauseTime =
    parseFloat(document.getElementById("pause_time").value) || 0;
  ...
  const startTime = document.getElementById("start_time").value || null;
  const endTime = document.getElementById("end_time").value || null;

  // Validate scheduling if both times are provided
  if ((startTime && !endTime) || (!startTime && endTime)) {
    logMessage(
      "Both start time and end time must be specified for scheduling",
      "error"
    );
    return;
  }
...
async function loadPlaylist(playlistName) {
  try {
    ...
    const response = await fetch(
      `/get_playlist?name=${encodeURIComponent(playlistName)}`
    );
    ...
    refreshPlaylistUI();
    logMessage(
      `Loaded playlist: "${playlistName}" with ${playlist.length} file(s).`
    );
  } catch (err) {
...
async function savePlaylist() {
  ...
  try {
    ...
    if (result.success) {
      logMessage(
        `Playlist "${name}" with ${playlist.length} patterns saved`,
        LOG_TYPE.SUCCESS
      );
...
function populatePlaylistDropdown() {
  return fetch("/list_all_playlists")
    ...
    .then((playlists) => {
      ...
      logMessage(
        "Playlist dropdown populated, event listener attached, and saved playlist restored."
      );
    })
...
async function confirmRenamePlaylist() {
  ...
  if (newName === currentName) {
    logMessage(
      "New playlist name is the same as the current name. No changes made.",
      LOG_TYPE.WARNING
    );
  ...
  try {
    ...
    if (createResult.success) {
      ...
      if (deleteResult.success) {
        ...
        logMessage(
          `Failed to delete old playlist: ${deleteResult.error}`,
          LOG_TYPE.ERROR
        );
      }
    } else {
      logMessage(
        `Failed to rename playlist: ${createResult.error}`,
        LOG_TYPE.ERROR
      );
    }
  } catch (error) {
...
function toggleSaveCancelButtons(show) {
  const actionButtons = document.querySelector(
    "#playlist-editor .action-buttons"
  );
  if (actionButtons) {
    // Show/hide all buttons except Save and Cancel
    actionButtons
      .querySelectorAll("button:not(.save-cancel)")
      .forEach((button) => {
...
function detectPlaylistChanges() {
  isPlaylistChanged =
    JSON.stringify(originalPlaylist) !== JSON.stringify(playlist);
...
function toggleSecondaryButtons(containerId, onShowCallback = null) {
  ...
  const previousActionButtons =
    container.previousElementSibling?.classList.contains("action-buttons")
...
async function saveToPlaylist() {
  ...
  try {
    ...
    if (result.success) {
      logMessage(
        `Pattern "${selectedFile}" successfully added to playlist "${playlist}".`,
        LOG_TYPE.SUCCESS
      );
      ...
      logMessage(
        `Failed to add pattern to playlist: ${result.error}`,
        LOG_TYPE.ERROR
      );
    }
  } catch (error) {
...
async function changeSpeed() {
  ...
  if (result.success) {
    ...
    ).textContent = `Current Speed: ${speed}`;
...
function closeStickySection(sectionId) {
  ...
  if (section) {
    ...
    if (sectionId === "playlist-editor") {
      document
        .querySelectorAll("#all_playlists .playlist-item")
        .forEach((item) => {
          item.classList.remove("selected");
        });
    }
    ...
    if (sectionId === "pattern-preview-container") {
      document
        .querySelectorAll("#theta_rho_files .file-item")
        .forEach((item) => {
          item.classList.remove("selected");
        });
    }
  ...
  document.querySelectorAll(".fullscreen-button").forEach((button) => {
    button.addEventListener("click", function () {
      ...
      if (stickySection) {
        ...
        document
          .querySelectorAll(
            ".sticky:not(#currently-playing-container):not(#image-converter)"
          )
          .forEach((section) => {
            if (section !== stickySection) {
              ...
              const otherFullscreenButton =
                section.querySelector(".fullscreen-button");
              if (otherFullscreenButton) {
                otherFullscreenButton.innerHTML =
                  '<i class="fa-solid fa-expand"></i>'; // Enter fullscreen icon/text
              }
            }
          });
        ...
        console.error(
          "Error: Fullscreen button is not inside a sticky section."
        );
      }
    });
  });
...
function loadSettingsFromCookies() {
  ...
  if (runMode !== "") {
    const runModeElement = document.querySelector(
      `input[name="run_mode"][value="${runMode}"]`
    );
    if (runModeElement) {
...
function switchTab(tabName, tabGroup = "main") {
  ...
  document
    .querySelectorAll(`.tab-button[data-group="${tabGroup}"]`)
    .forEach((button) => {
      button.classList.remove("active");
    });
...
async function loadWledIp() {
  ...
  if (!savedIp) {
    ...
    try {
      ...
      logMessage(
        `Error fetching WLED IP from backend: ${error.message}`,
        LOG_TYPE.ERROR
      );
    }
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const activeImageConverterTab =
    getCookie("activeTab-image-converter") || "dot";
...
function updateCurrentlyPlayingUI(status) {
  ...
  if (status.progress) {
    ...
    const remainingText =
      remaining_time === null
        ? "calculating..."
        : formatSecondsToHMS(remaining_time);
...
function updateSkipButtonVisibility(status) {
  ...
  if (skipButton) {
    ...
    const isPlaylistPlaying =
      status && status.playlist && status.playlist.next_file;
